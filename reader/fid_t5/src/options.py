...
class Options():
    ...
    def add_reader_options(self):
        ...
        self.parser.add_argument('--model_name', type=str, default=None,
                        help='pretrained T5 name; overrides t5-<model_size> for a fresh run')
        self.parser.add_argument('--balance_labels', action='store_true',
                        help='sample claim labels with equal expected frequency')
        self.parser.add_argument('--selection_metric', choices=['accuracy', 'macro_f1'],
                        default='accuracy', help='metric used to select best_dev')
        self.parser.add_argument('--binary_labels', action='store_true',
                        help='restrict predictions and macro-F1 to SUPPORTS/REFUTES')
